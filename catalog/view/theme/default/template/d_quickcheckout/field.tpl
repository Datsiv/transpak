<script type="text/html" id="field_template">
	...
	<div id="<%= model.config.id %>_<%= f.id %>_input"
		...
		<div class="<%= col_left %>">
			<label class="control-label" for="<%= model.config.id %>_<%= f.id %>">
				<span class="text1" <%= f.tooltip ? 'data-toggle="tooltip"' : '' %> title="<%= f.tooltip %>"><%= f.title %></span>
			</label>
		</div>
		<div class="<%= col_right %>">
	...
	<div id="<%= model.config.id %>_<%= f.id %>_input"
		...
		<div class="<%= col_left %>">
			<label class="control-label" for="<%= model.config.id %>_<%= f.id %>">
				<span class="text1" <%= f.tooltip ? 'data-toggle="tooltip"' : '' %> title="<%= f.tooltip %>"> <%= htmlDecode(f.title) %></span>
			</label>
		</div>
		<div class="<%= col_right %>">
			<div class="input-group">
				<input type="text"
					   ...
					   class="form-control mask <%= f.type %> validate <%= require ? 'required' : 'not-required' %> <%= f.id %>"
					   autocomplite="on"
					   data-mask="<%=f.mask%>"
	...
	<div id="<%= model.config.id %>_<%= f.id %>_input"
		 class="text-input form-group  sort-item <%= display ? '' : 'hidden' %> <%= f.class ? f.class : '' %> <%= require ? 'required' : '' %> conf_form_styl22"
		 data-sort="<%= f.sort_order %>">
		<div class="col-xs-12">
			<label class="control-label" for="<%= model.config.id %>_<%= f.id %>">
				<span class="text1" <%= f.tooltip ? 'data-toggle="tooltip"' : '' %> title="<%= f.tooltip %>"> <%= htmlDecode(f.title) %></span>
		...
		<div class="col-xs-12">
		          <textarea
						  name="<%= model.config.id %>.<%= f.id %>"
						  id="<%= model.config.id %>_<%= f.id %>"
						  class="form-control validate <%= require ? 'required' : 'not-required' %> <%= f.type %> <%= f.id %> texter_heidch"
						  autocomplite="on"
			<% if(Number(config.design.placeholder)) {  %>placeholder="<%= require ? '*' : '' %> <%= htmlDecode(f.title).replace(':', '') %>"<% } %>
			<%= setValidateRules(f.error) %> ><%= model[model.config.id][f.id] %>
			</textarea>
		</div>
	</div>
	...
	<div id="<%= model.config.id %>_<%= f.id %>_input"
		 class="text-input form-group  sort-item <%= display ? '' : 'hidden' %> <%= f.class ? f.class : '' %> <%= require ? 'required' : '' %>"
		 data-sort="<%= f.sort_order %>">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 clas_pad" style="padding-right: 0">
			<div class="col-sm-12">
				<label class="control-label" for="<%= model.config.id %>_<%= f.id %>">
					<span class="text1" <%= f.tooltip ? 'data-toggle="tooltip"' : '' %> title="<%= f.tooltip %>"> <%= htmlDecode(f.title) %></span>
				</label>
