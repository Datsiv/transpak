<?php
class ControllerCatalogAkcii extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('catalog/akcii');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			$this->response->redirect($this->url->link('catalog/akcii', 'token=' . $this->session->data['token'], 'SSL'));
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_akcii->addNews($this->request->post);
			...
			$this->response->redirect($this->url->link('catalog/akcii', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_akcii->editNews($this->request->get['news_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $news_id) {
				$this->model_catalog_akcii->deleteNews($news_id);
			}
	...
	private function getList() {
		...
		$data['breadcrumbs'][] = array(
			'href'      => $this->url->link('catalog/akcii', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);
	
		$data['add'] = $this->url->link('catalog/akcii/add', 'token=' . $this->session->data['token'], 'SSL');
		$data['delete'] = $this->url->link('catalog/akcii/delete', 'token=' . $this->session->data['token'], 'SSL');
		$data['setting'] = $this->url->link('catalog/akcii/setting', 'token=' . $this->session->data['token'], 'SSL');
		...
		$results = $this->model_catalog_akcii->getNewsList($filter_data);
    	...
    	foreach ($results as $result) {
			...
			$data['news'][] = array(
				...
				'edit'      	=> $this->url->link('catalog/akcii/edit', 'token=' . $this->session->data['token'] . '&news_id=' . $result['news_id'], 'SSL')
			);
		}
		...
		$data['sort_title'] = $this->url->link('catalog/akcii', 'token=' . $this->session->data['token'] . '&sort=nd.title' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('catalog/akcii', 'token=' . $this->session->data['token'] . '&sort=n.date_added' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/akcii', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/akcii_list.tpl', $data));
    ...
    if ($this->config->get('config_editor_default')) {
		...
		if (!isset($this->request->get['news_id'])) {
			$data['action'] = $this->url->link('catalog/akcii/add', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/akcii/edit', 'token=' . $this->session->data['token'] . '&news_id=' . $this->request->get['news_id'], 'SSL');
		}
	
		$data['cancel'] = $this->url->link('catalog/akcii', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['news_description'])) {
			...
			$data['news_description'] = $this->model_catalog_akcii->getNewsDescriptions($this->request->get['news_id']);
		} else {
		...
		if (isset($this->request->post['meta_keyword'])) {
			...
			$data['meta_keyword'] = $this->model_catalog_akcii->getNewsDescriptions($this->request->get['news_id']);
		} else {
		...
		if (isset($this->request->post['news_store'])) {
			...
			$data['news_store'] = $this->model_catalog_akcii->getNewsStores($this->request->get['news_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('catalog/akcii_form.tpl', $data));
	...
	public function setting() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateSetting()) {
			$this->model_setting_setting->editSetting('news_setting', $this->request->post);
				if (isset($this->request->post['news_url'])) {
					$this->model_catalog_akcii->setNewsListUrl($this->request->post['news_url']);
				}	
			$this->session->data['success'] = $this->language->get('text_success');

			$this->cache->delete('news_setting');

			$this->response->redirect($this->url->link('catalog/akcii/setting', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$data['breadcrumbs'][] = array(
			'href'      => $this->url->link('catalog/akcii/setting', 'token=' . $this->session->data['token'], 'SSL'),
		...
		$news_url = $this->model_catalog_news->getNewsListUrl('information/akcii');
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'catalog/akcii')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
